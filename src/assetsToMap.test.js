...
function cleanResultFiles(){
    ...
    fs.unlink(files+".json", (err) => { if (err) { } });
    fs.unlink(files+".yaml", (err) => { if (err) { } });
    fs.unlink(files+".toml", (err) => { if (err) { } });
}

function cleanSourceFilesMock() {
    fs.unlink("./src/mock/mock.js", (err) => { if (err) { } });
    fs.unlink("./src/mock/mock2.css", (err) => { if (err) { } });
}

function cpSourceFilesMock() {
    fs.copyFileSync('./src/mock/sourceFiles/mock.js', './src/mock/mock.js', (err) => { if (err) throw err; });
    fs.copyFileSync('./src/mock/sourceFiles/mock2.css', './src/mock/mock2.css', (err) => { if (err) throw err; });
}
