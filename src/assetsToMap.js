...
AssetsToMap.prototype.writeFile = function() {
  ...
  for (const key in this.files) {
    ...
    switch (this.format) {
      case "toml":
        resultStr += `${key} = '''
                ${fs
                  .readFileSync(this.sourcePath + this.files[key])
                  .toString("UTF-8")}'''
                \n`;
        break;
      case "yaml":
    ...
    this.logger.info(
      `${prefixLog}${this.sourcePath}${this.files[key]} added to ${
        this.outputPath
      }${this.filename}`
    );
    ...
    if (this.deleteFiles) {
      fs.unlink(this.sourcePath + this.files[key], err => {
        ...
        this.logger.info(
          `${prefixLog}${this.sourcePath}${this.files[key]} was deleted`
        );
      });
    }
  }
  ...
  fs.writeFileSync(
    this.outputPath + this.filename + "." + this.format,
    new Buffer(
      this.format === "yaml" || this.format === "toml"
        ? resultStr
  ...
  this.logger.info(
    `${prefixLog}${this.outputPath}${this.filename} was created`
  );
